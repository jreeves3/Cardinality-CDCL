...
then # CNF input formula
  ...
  ./tools/pairwise tmp/card.knf > tmp/encoded.cnf

  # Permute extracted KNF
  python3 tools/permute.py -c tmp/encoded.cnf -o tmp/permuted.cnf

  echo "Start Cadical"
  
  # Run cadical on the permuted CNF, with a 5000 second timeout
  ./cadical/build/cadical tmp/permuted.cnf tmp/solve.drat --no-binary -t 5000 > tmp/cadical.out
  ...
  ./tools/pairwise $1 > tmp/encoded.cnf

  # Permute reencoded CNF formula
  python3 tools/permute.py -c tmp/encoded.cnf -o tmp/permuted.cnf
