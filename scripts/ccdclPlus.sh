...
then # CNF input formula
  ...
  # Attach encoded klauses to KNF
  ./tools/konly tmp/card.knf > tmp/encode.cnf

  cat tmp/card.knf tmp/encode.cnf > tmp/full.knf

  VAR=$(grep "p cnf" tmp/encode.cnf | awk '{print $3}')
  CLS1=$(grep "p cnf" tmp/encode.cnf | awk '{print $4}')
  CLS2=$(grep "p knf" tmp/card.knf | awk '{print $4}')
  ...
  # Permute extracted KNFPlus
  python3 tools/permute.py -c tmp/full.knf -o tmp/permuted.knf
  ...
  then
    ...
    # UNSAT case
      ...
      ./tools/derivation tmp/card.knf > tmp/deriv.drat
      cat tmp/deriv.drat tmp/solve.drat > tmp/proof.drat

      echo "Start Drat Checking"
      ./drat-trim/drat-trim $1 tmp/proof.drat > tmp/drat-check.out
  ...
  # Attach encoded klauses to KNF
  ./tools/konly $1 > tmp/encode.cnf

  cat $1 tmp/encode.cnf > tmp/full.knf

  VAR=$(grep "p cnf" tmp/encode.cnf | awk '{print $3}')
  CLS1=$(grep "p cnf" tmp/encode.cnf | awk '{print $4}')
  CLS2=$(grep "p knf" $1 | awk '{print $4}')
  grep -v "p cnf \|p knf" tmp/full.knf > tmp/full-nohead.knf
  CLS=$(($CLS1+$CLS2))
  printf 'p knf %s %s\n' "$VAR $CLS" > tmp/temp1.txt
  cat tmp/temp1.txt tmp/full-nohead.knf > tmp/full.knf
